...
parser.add_argument('--arch', '-a', metavar='ARCH', default='LEncoder1',  help='model architecture')#GEncoder1   LEncoder1
...
parser.add_argument('--savepath',default='CKPT/Ablation2/VMamba_stage2_leaky_relu_double_ffm', help='path to save ckpt')
...
def main():
    ...
    # checkpoint = torch.load('/root/autodl-tmp/mamba_s/CKPT/Ablation/VMamba_stage1_leaky_relu_double_ffm/BSD500/GEncoder1_adam_3000000epochs_epochSize6000_b16_lr0.0005_posW0.003_24_11_22_13_47/model_best.tar')
...
def train(train_loader, model, optimizer, epoch, train_writer, init_spixl_map_idx, xy_feat):
    ...
    for i, (input, label, edge) in enumerate(train_loader):
        ...
        output, pred_edge = model(input_gpu)
        ...
        edge_loss = cross_entropy_loss_edge( pred_edge, edge.to(device))
