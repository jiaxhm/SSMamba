...
import models
import datasets
from loss import compute_semantic_pos_loss,cross_entropy_loss_edge
import datetime
...
parser.add_argument('--arch', '-a', metavar='ARCH', default='LEncoder1',  help='model architecture')#    LEncoder1   GEncoder1
# parser.add_argument('--data', metavar='DIR',default='/root/root/autodl-tmp/project/data_preprocessing/DUMP_DIR', help='path to input dataset')
parser.add_argument('--data', metavar='DIR',default='/root/autodl-tmp/mamba_s/data_preprocessing/DUMP_DIR', help='path to input dataset')
parser.add_argument('--savepath',default='CKPT/VMamba_same_LELU_multiloss', help='path to save ckpt')
...
def main():
    ...
    input_transform = transforms.Compose([
        flow_transforms.PhotometricDistort(),
...
def train(train_loader, model, optimizer, epoch, train_writer, init_spixl_map_idx, xy_feat):
    ...
    for i, (input, label, edge) in enumerate(train_loader):
        ...
        global_sup, global_edge, local_sup, local_edge = model(input_gpu)  # global_sup, global_edge, local_sup, local_edge    #   output, pred_edge
        slic_loss1, loss_sem1, loss_pos1 = compute_semantic_pos_loss( global_sup, LABXY_feat_tensor,
                                                                pos_weight= args.pos_weight, kernel_size=args.downsize)
        
        slic_loss2, loss_sem2, loss_pos2 = compute_semantic_pos_loss( local_sup, LABXY_feat_tensor,
                                                                pos_weight= args.pos_weight, kernel_size=args.downsize)
        
        edge_loss1 = cross_entropy_loss_edge( global_edge, edge.to(device))
        edge_loss2 = cross_entropy_loss_edge( local_edge, edge.to(device))
        
        slic_loss = slic_loss1 + slic_loss2
        loss_sem = loss_sem1 + loss_sem2
        loss_pos = loss_pos1 + loss_pos2
        edge_loss = edge_loss1 +  edge_loss2
        
        
        global_loss = slic_loss1 + edge_loss1
        local_loss = slic_loss2 + edge_loss2
    ...
    if epoch % args.record_freq == 0:
        ...
        curr_spixl_map = update_spixl_map(init_spixl_map_idx,local_sup)   #  output
...
def validate(val_loader, model, epoch, val_writer, init_spixl_map_idx, xy_feat):
    ...
    for i, (input, label,_) in enumerate(val_loader):
        ...
        # compute output
        with torch.no_grad():
            global_sup, local_sup = model(input_gpu)
            slic_loss1, loss_sem1, loss_pos1 = compute_semantic_pos_loss(global_sup, LABXY_feat_tensor,
                                        pos_weight=args.pos_weight, kernel_size=args.downsize)
            slic_loss2, loss_sem2, loss_pos2 = compute_semantic_pos_loss(local_sup, LABXY_feat_tensor,
                                        pos_weight=args.pos_weight, kernel_size=args.downsize)
